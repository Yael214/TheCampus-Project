...
/**
 * Reset Component
 * Handles the password recovery flow via Firebase Auth.
 * Includes error handling and a resend mechanism for undelivered emails.
 */
function Reset() {
  ...
  const sendResetRequest = async (targetEmail) => {
    ...
    try {
      ...
      setMessage(
        "קישור לאיפוס הסיסמה נשלח לתיבת המייל שלך. בדוק/י גם בתיקיית הספאם.",
      );
      setEmailSent(true);
    } catch (err) {
      console.error("Password reset error:", err);

      // Map Firebase Auth error codes to user-friendly Hebrew messages
        ...
        {!emailSent ? (
          ...
          <div style={{ textAlign: "center", marginTop: "20px" }}>
            <p style={{ marginBottom: "15px" }}>
              המייל נשלח אל: <strong>{email}</strong>
            </p>
