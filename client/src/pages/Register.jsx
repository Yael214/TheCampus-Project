...
/**
 * Register Component
 * Handles new user registration including complex form validation, 
 * file uploads (profile image, study approval), and address geocoding.
 */
function Register() {
  ...
  const validateRegister = () => {
    ...
    if (!formData.idNumber) {
      ...
      !/^\d+$/.test(formData.idNumber) ||
      formData.idNumber.length !== 9
    ) {
    ...
    if (!formData.phone) {
      ...
      tempErrors.phone =
        "מספר טלפון לא תקין (צריך להכיל 10 ספרות ולהתחיל ב 05)";
    }
    ...
    if (!formData.studyApproval) {
      tempErrors.studyApproval = "חובה להעלות אישור לימודים";
    } else {
      if (formData.studyApproval.type !== "application/pdf") {
        tempErrors.studyApproval = "ניתן להעלות רק קובץ PDF";
      } else if (
        formData.studyApproval &&
        formData.studyApproval.size > 2 * 1024 * 1024
      ) {
        tempErrors.studyApproval = "הקובץ גדול מדי (מקסימום 2MB)";
      }
    }
  ...
  return (
    <div className="auth-page">
      ...
      <div className="container" style={{ maxWidth: "500px" }}>
        ...
        <label>
          <span className="required">*</span>שם מלא
        </label>
        <input
          type="text"
          name="fullName"
        ...
        <label>
          <span className="required">*</span>תעודת זהות
        </label>
        <input
          type="text"
          name="idNumber"
        ...
        <label>
          <span className="required">*</span>סיסמה
        </label>
        <input
          type="password"
          name="password"
        ...
        <label>
          <span className="required">*</span>כתובת
        </label>
        <AddressInput
          name="address"
        ...
        <label>
          <span className="required">*</span>תחום לימודים
        </label>
        <input
          type="text"
          name="studyField"
          className={errors.studyField ? "input-error" : ""}
          onChange={handleInputChange}
        />
        {errors.studyField && (
          <span className="error-msg">{errors.studyField}</span>
        )}

        <label>
          <span className="required">*</span>שנת לימודים
        </label>
        <input
          type="text"
          name="year"
        ...
        {/* Study Approval Upload (PDF only) */}
        <label>
          <span className="required">*</span>אישור לימודים
        </label>
        <input
          type="file"
          name="studyApproval"
          accept=".pdf"
          className={errors.studyApproval ? "input-error" : ""}
          onChange={handleInputChange}
        />
        {errors.studyApproval && (
          <span className="error-msg">{errors.studyApproval}</span>
        )}
        ...
        >
          {isSubmitting ? "נרשם..." : "סיום"}
        </button>
        <div style={{ textAlign: "center", marginTop: "10px" }}>
          <Link to="/login" style={{ cursor: "pointer", color: "#6B7280" }}>
            ביטול
          </Link>
        </div>
      </div>
