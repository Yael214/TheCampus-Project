...
function Forums() {
  ...
  // Fetch live forum posts using the dedicated custom hook
  const {
    posts: apiPosts,
    loading: postsLoading,
    error: postsError,
  } = useForumPosts(safeForumId) || { posts: [], loading: false, error: null };

  // Synchronize post data from the database, ensuring the 'likes' count is properly initialized
  useEffect(() => {
  ...
  useEffect(() => {
    // Fetch forum metadata. Includes fallback logic to handle both capitalized and lowercase document IDs
    const fetchForumDetails = async () => {
      if (!safeForumId) return;
      try {
        const formattedId =
          safeForumId.charAt(0).toUpperCase() + safeForumId.slice(1);
  ...
  if (!safeForumId) {
    return (
      <div className="p-8 text-center text-gray-500">טוען נתוני פורום...</div>
    );
  }

  return (
    <div
      className="w-full h-full p-8 flex flex-col items-start justify-start"
      dir="rtl"
    >
      {/* Header Area */}
        ...
        <section className="flex-[2] overflow-y-auto space-y-4 pl-2 text-right">
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              ...
              className="bg-[#4F46E5] text-white px-5 py-2 rounded-xl font-bold text-xs flex items-center gap-1.5 cursor-pointer hover:bg-indigo-700 transition-colors"
            >
              <span>פוסט חדש</span>
          ...
          {postsLoading ? (
            ...
            <div className="bg-white p-16 rounded-3xl text-center border border-white shadow-sm flex flex-col items-center justify-center w-full">
              ...
              <h3 className="font-bold text-[#2C3E7A] text-lg mb-1">
                הפורום עדיין ריק
              </h3>
              <p className="text-gray-400 text-sm max-w-xs">
                אין עדיין פוסטים בקורס זה. תהיי הראשונה לפרסם פוסט!
              </p>
            </div>
          ) : (
            posts.map((post) => (
