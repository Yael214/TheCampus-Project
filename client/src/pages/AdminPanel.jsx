 ...
 * Renders a dashboard for system administrators to manage users.
 * Capabilities include searching users, toggling block statuses, and promoting users to admins.
...
const AdminPanel = () => {
  ...
  if (currentUser?.role !== "admin") {
    return (
      <div className="p-6 text-center text-red-500 font-bold" dir="rtl">
        אין גישה. העמוד מורשה למנהלים בלבד.
      </div>
    );
  }
  ...
  const handleMakeAdmin = async (userId) => {
    ...
    const confirmAdmin = window.confirm(
      "האם את בטוחה שברצונך להפוך משתמש זה למנהל מערכת? לא יהיה ניתן לבטל זאת מכאן.",
    );
    ...
    try {
      ...
      alert(
        "המשתמש הוגדר כמנהל בהצלחה! (כדי לראות את השינוי, עליו לרענן את האפליקציה).",
      );
    } catch (error) {
  ...
  // Filter users by search query and sort the list (Admins appear first)
  ...
  return (
    <div className="max-w-5xl mx-auto p-6" dir="rtl">
      <h1 className="text-3xl font-bold text-[#2C3E7A] mb-6">
            ...
            {filteredUsers.length > 0 ? (
              filteredUsers.map((user) => (
                ...
                >
                  ...
                  <td className="p-4">
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-semibold ${user.role === "admin" ? "bg-purple-100 text-purple-700" : "bg-gray-100 text-gray-600"}`}
                    >
                  ...
                  <td className="p-4">
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-semibold ${user.isBlocked ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                    >
                  ...
                  <td className="p-4">
                    <div className="flex gap-2 items-center">
                      ...
                      {user.id === currentUser.uid ? (
                        <span className="text-gray-400 text-sm font-medium px-2 italic">
                          החשבון שלך
                        </span>
                      ) : user.role === "admin" ? (
                        <span className="text-gray-400 text-sm font-medium px-2 italic">
